...
class DetSolver(BaseSolver):
    ...
    def fit(self, ):
        ...
        if dist_utils.is_main_process():
            print("\n" + "="*50)
            print("Training completed. Evaluating best models...")
            print("="*50)
            ...
            if stg2_path:
                if stg2_path.exists():
                    print(f"\n[Stage2] Loading best model from {stg2_path}")
                    self.load_resume_state(str(stg2_path))
                    
                    print("\nEvaluating best stage2 model:")
                    self.val()
            else:
                if stg1_path.exists():
                    print(f"[Stage1] Loading best model from {stg1_path}")
                    self.load_resume_state(str(stg1_path))
                    
                    print("\nEvaluating best stage1 model:")
                    self.val()
        ...
        if log_file.exists():
                print("\nGenerating loss curves from training log...")
                visualizer.plot_loss_curves(log_file)
                print("Loss curves saved to output directory.")
        ...
        if dist_utils.is_main_process():
            ...
            for k, v in test_stats.items():
                if isinstance(v, (list, tuple)):
                    ...
                    print(f"  - {k}: {round(v, 4)}")
            if coco_evaluator is not None and hasattr(coco_evaluator, 'summary'):
                coco_evaluator.summary()
