...
def check_nan_inf(tensor, name="Tensor", raise_exception=True, verbose=True):
    
    if not torch.is_tensor(tensor):
        if verbose:
            print(f"{name} is not a PyTorch Tensor, skipping check.")
        return True
    ...
    # 根据参数决定是否抛出异常
    if not is_healthy and raise_exception:
        raise RuntimeError(f"Numerical instability detected in {name}. " "Contains NaN or Inf values. Training halted.")
